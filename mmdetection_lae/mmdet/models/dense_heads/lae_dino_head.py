...
class LAEDINOHead(DINOHead):
    ...
    def loss_by_psedo_instances(self, hidden_states: Tensor, references: List[Tensor],
             memory_text: Tensor, text_token_mask: Tensor,
             enc_outputs_class: Tensor, enc_outputs_coord: Tensor,
             batch_data_samples: SampleList, origin_pseudo_data_samples: SampleList, dn_meta: Dict[str, int]) -> dict:
        """Perform forward propagation and loss calculation of the detection
        head on the queries of the upstream network.
        ...
        for data_sample in origin_pseudo_data_samples:
            batch_gt_instances.append(data_sample.gt_instances)
